...
**月配列2-263をベースに、濁音・半濁音を「清音 + ゛」の後置に統合したかな配列。標準 US（ANSI）キーボードの英字 3 行 30 キーで動き、同時押しを使いません。**
...
| 濁音・半濁音・小書きの母音 | 清音（親文字）→ ゛ の後置。半濁音は ゛゛（例: が = か→゛、ぱ = は→゛→゛、ぁ = あ→゛。例外は ぅ = う→゛→゛）。ゃ・ゅ・ょ は専用キー（ょ 単打、ゃ・ゅ ☆面） | 清音の打鍵数 +1 / +2 |
